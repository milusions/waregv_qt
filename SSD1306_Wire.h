...
public:
    ...
    void begin() {
        ...
        // Universal initialization sequence compatible with SSD1306 & SH1106
        sendCommand(0xAE); // Display OFF
        sendCommand(0xD5); sendCommand(0x80); // Display clock divide ratio
    ...
    // CRITICAL FIX: Page-by-page rendering with +2 column offset for SH1106 / SSD1306 compatibility
    void display() {
        for (uint8_t page = 0; page < 8; page++) {
            sendCommand(0xB0 + page); // Set page address (0 to 7)
            sendCommand(0x02);        // Set lower column address offset (+2 pixels for SH1106)
            sendCommand(0x10);        // Set higher column address offset (0)

            // Stream 128 bytes per page in 16-byte Wire chunks
            for (uint8_t chunk = 0; chunk < 128; chunk += 16) {
                Wire.beginTransmission(OLED_I2C_ADDR);
                Wire.write(0x40); // Data stream byte header
